...
export class AttendanceCommands {
    @SlashCommand({name: "attendance", description: "Get your current attendance"})
    async AttendanceCommand(@Context() [interaction]: SlashCommandContext) {
        await interaction.reply("Attendance command");
    }

    @SlashCommand({name: "attendance-leaderboard", description: "Show the top 5 members by attendance hours"})
